...
const createUserArticleHTML = user => {
  ...
  article.innerHTML = `<img src="" alt=""><h1></h1><h2></h2>
...
const createLiPost = post => {
  ...
  li.innerHTML = `<div class="col col-1" data-label="Post Id"></div>
...
const createLiPostExt = post => {
  let li = document.createElement('li')
  li.classList.add('table-row')
  li.innerHTML = `<div class="col col-1v2" data-label="Post Id"></div>
...
window.addEventListener('load', () => {
  ...
  if (window.location.href == `${window.location.origin}/${prefixGithubPages}index.html`) {
    ...
    history.pushState(
      ...
      null,
      `${window.location.origin}/${prefixGithubPages}#home`
    )
  }
  onRouteChanged()
})
  ...
  anchor.addEventListener('click', e => {
    ...
    if (flag) {
      history.pushState(
        ...
        null,
        `https://${window.location.host}/${prefixGithubPages}` + e.target.hash
      )
    } else {
